                    ...
                    <div class="col-sm-4 col-md-2 col-md-offset-5 col-sm-offset-4" >
                        ...
                        <div class="btn-group pull-left" style="text-align: left">
                            ...
                            <ul class="dropdown-menu" style="z-index: 1005; left: auto" >
                                <li><a href="<c:url value='/events/${event.id}/edit'/>"><i class="fa fa-fw fa-pencil"></i> <spring:message code="jsp.event.edit"/></a></li>
                                <li><a href="<c:url value='/events/${event.id}/members'/>"><i class="fa fa-fw fa-group"></i> <spring:message code="jsp.event.viewmembers"/></a></li>
                                    <c:if test="${event.status eq 'OPEN' and fn:length(results) eq 0}">
                                    <li><a href="<c:url value='/events/${event.id}/routes'/>"><i class="fa fa-fw fa-flag-checkered"></i> <spring:message code="jsp.route.select"/></a></li>
                                    <li><a href="<c:url value='/events/${event.id}/segments'/>"><i class="fa fa-fw fa-flag-checkered"></i> <spring:message code="jsp.segment.select"/></a></li>
                                    </c:if>
                                <li>
                    ...
                    <c:choose>
                        <c:when test="${event.routeId eq null and event.segmentId eq null}">
                            <div class="panel panel-default panel-body" style="text-align: center">
                                <a class="btn btn-primary" style="margin: 5px;" href="<c:url value='/events/${event.id}/routes'/>">
                                    <i class="fa fa-fw fa-flag-checkered"></i> <spring:message code="jsp.route.select"/></a>
                                <a class="btn btn-primary" style="margin: 5px;" href="<c:url value='/events/${event.id}/segments'/>">
                                    <i class="fa fa-fw fa-flag-checkered"></i> <spring:message code="jsp.segment.select"/></a>
                            </div>
                        </c:when>
                        <c:otherwise>
        ...
        <div class=" col-sm-5 col-md-4  ">
            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    ...
                    <c:set var="addresult" value="true"/>
                    <c:forEach var="result" items="${results}">
                        <c:if test="${result.owner.username eq user.username}">
                            <c:set var="addresult" value="false"/>
                        </c:if>
                    </c:forEach>
                    <c:if test="${event.status eq 'OPEN' and (event.routeId ne null or event.segmentId ne null) and addresult eq 'true'}">
                        <button class="btn btn-success btn-xs pull-right" data-toggle="modal" data-target="#myModal">
                            <span class="fa fa-plus" style="margin-right: 4px;"></span> <spring:message code="jsp.event.addresult"/></button>
                        <!-- Modal -->
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        ...
                                        <h4 class="modal-title" id="myModalLabel"><spring:message code="jsp.form.selectgpx"/></h4>
                                    </div>
                                    <div class="modal-body">
                                        <c:url value='/events/${event.id}/practices' var="practicePostUrl"/>
                                        <form:form method="POST" commandName="gpxForm" action="${practicePostUrl}" enctype="multipart/form-data">
                                            <form:input type="file" path="file" />
                                            <div class="modal-footer">
                                                <button class="btn btn-small btn-success" type="submit" value=""><span class="fa fa-upload" style="margin-right: 4px;"></span> <spring:message code="jsp.form.uploadfile"/></button>
                                            </div>
                                        </form:form>
                                    </div>
                                </div>
